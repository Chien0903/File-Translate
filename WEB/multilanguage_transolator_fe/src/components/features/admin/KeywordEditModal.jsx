...
const KeywordEditModal = ({ keyword, onChange, onSave, onClose }) => {
  ...
  const handleChange = (code, value) =>
    onChange({ ...keyword, translations: { ...translations, [code]: value } });
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50" style={{ backgroundColor: "rgba(255, 255, 255, 0.7)" }} onClick={onClose}>
      <div className="bg-white p-6 rounded-lg shadow-xl w-11/12 max-h-[90vh] overflow-auto text-center" style={{ maxWidth: "1100px" }} onClick={(e) => e.stopPropagation()}>
        <h3 className="text-lg text-indigo-700 font-bold mb-4 text-center">EDIT KEYWORD</h3>
        <form onSubmit={(e) => { e.preventDefault(); onSave(); }}>
          ...
          <div className="mt-6 flex justify-center gap-3">
            <button type="submit" className="px-6 py-2 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 transition-all duration-200 shadow-md hover:shadow-lg font-medium">
              Save Changes
            </button>
